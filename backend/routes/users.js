            ...
            if (fetchUser.verified === true) {
              ...
              Settings.find().then(document => {
                ...
                if (fetchUser.user_type === 'Admin') {
                  ...
                  Users.update({ _id: fetchUser._id},
                    { $set: { 'status': true }}
                  )
                  .then(result => {
                    res.status(200).json({
...
router.post('/resetPassword', (req, res, next) => {
  ...
  //       .then(result => {
        ...
        Users.find({ email_id: req.body.email })
          .then(user => {

            console.log(user);
            console.log(user[0].email_id)
            console.log(user[0].first_name);
              const token = jwt.sign(
                {
                  mailId: user[0].email_id,
                  userId: user[0]._id
                },
                'diaMond^B__ABuk?09(-_-)',
                {expiresIn: '30m'}
                              ...
                              <tr style="border-collapse:collapse"> 
                                ...
                                <table width="100%" cellspacing="0" cellpadding="0" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px"> 
                                  <tr style="border-collapse:collapse"> 
                                    ...
                                    <table width="100%" cellspacing="0" cellpadding="0" role="presentation" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px"> 
                                      <tr style="border-collapse:collapse"> 
                                        <td class="es-m-txt-c" align="center" style="padding:0;Margin:0"><h2 style="Margin:0;line-height:34px;mso-line-height-rule:exactly;font-family:'open sans', 'helvetica neue', helvetica, arial, sans-serif;font-size:28px;font-style:normal;font-weight:normal;color:#474745">Hey, ${user[0].first_name} ${ user[0].last_name }!</h2></td> 
                                      </tr>
                                      
                                      <tr style="border-collapse:collapse"> 
                                        <td class="es-m-txt-c" align="center" style="padding:0;Margin:0"><h2 style="Margin:0;line-height:34px;mso-line-height-rule:exactly;font-family:'open sans', 'helvetica neue', helvetica, arial, sans-serif;font-size:28px;font-style:normal;font-weight:normal;color:#474745">Your Mail Id, ${user[0].email_id}!</h2></td> 
                                      </tr>


                                      <tr style="border-collapse:collapse"> 
                                        <td class="es-m-txt-c" align="center" style="padding:0;Margin:0"><h2 style="Margin:0;line-height:34px;mso-line-height-rule:exactly;font-family:'open sans', 'helvetica neue', helvetica, arial, sans-serif;font-size:28px;font-style:normal;font-weight:normal;color:#474745">Your Password</h2></td> 
                                      </tr>
...
router.post('/changePassword', (req, res, next) => {
  ...
  bcrypt.hash(req.body.password, 10)
    .then(hash => {
      ...
      Users.update({ _id: req.userData.userId},
        { $set: { 'password': hash}}
      )
      .then(result => {
        res.status(201)
...
router.post('/logout', checkAuth, (req, res, next) => {
  ...
  Users.updateOne({ _id: req.userData.userId},
    { $set: { 'status': false }}
  )
  .then(result => {
    res.status(201)
